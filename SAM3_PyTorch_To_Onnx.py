...
# Hide all warnings during the ONNX export
warnings.filterwarnings("ignore")

def _run_spinner(description, stop_event):
    """This runs in a completely separate process to avoid PyTorch locking it."""
...
class VisionEncoderWrapper(nn.Module):
    ...
    def __init__(self, sam3_model: Sam3Model, device="cpu", image_height=1008, image_width=1008):
        super().__init__()
...
def export_vision_encoder(
    ...
    wrapper = VisionEncoderWrapper(model, device=device, image_height=image_height, image_width=image_width).to(device).eval()
...
def export_geometry_encoder(
    ...
    fpn2_h = image_height // 14
    fpn2_w = image_width // 14
...
def print_onnx_summaries(models_dir):
    """Parses and prints the shapes of all exported ONNX models."""
    print('\n' + '='*40 + '\n ONNX MODEL SUMMARIES \n' + '='*40)
    results = []
    ...
    if os.path.exists(models_dir):
        ...
        # Start the background timer for the summary process
        timer = ProgressTimer(f"Parsing {len(files)} ONNX models")
        timer.start()
        
        try:
            for f in files:
                try:
                    m = onnx.load(os.path.join(models_dir, f))
                    rows = []
                    
                    # Use zip_longest to handle models with different numbers of inputs vs outputs safely
                    for i, o in itertools.zip_longest(m.graph.input, m.graph.output):
                        in_name = textwrap.fill(i.name, width=25) if i else "-"
                        if i:
                            in_shape_raw = [d.dim_value or d.dim_param for d in i.type.tensor_type.shape.dim]
                            in_shape = textwrap.fill(str(in_shape_raw), width=35)
                        else:
                            in_shape = "-"
                            
                        out_name = textwrap.fill(o.name, width=25) if o else "-"
                        if o:
                            out_shape_raw = [d.dim_value or d.dim_param for d in o.type.tensor_type.shape.dim]
                            out_shape = textwrap.fill(str(out_shape_raw), width=35)
                        else:
                            out_shape = "-"
                        
                        rows.append([in_name, in_shape, out_name, out_shape])
                    
                    table = tabulate.tabulate(rows, headers=['Input Name', 'Input Shape', 'Output Name', 'Output Shape'], tablefmt='fancy_grid')
                    results.append(f'\nModel: {f}\n{table}')
...
def main():
    parser = argparse.ArgumentParser(description="Export SAM3 model to ONNX")
    parser.add_argument(
        "--module", type=str, choices=["vision", "text", "geometry", "decoder"], default=None
    )
    ...
    parser.add_argument("--size", type=int, default=1008, help="Target image size (must be a multiple of 14)")
    args = parser.parse_args()

    if not args.module and not args.all:
        parser.error("Please specify --module or --all")

    # Interactive validation for multiple of 14
    target_size = args.size
    if target_size % 14 != 0:
        viable_size = round(target_size / 14) * 14
        print(f"⚠️ Warning: Requested size {target_size} is not a multiple of 14.")
        print(f"✅ The closest viable resolution is {viable_size}x{viable_size}.")
        
        while True:
            choice = input("Do you want to continue Onnx Export with this resolution? (Yes/No): ").strip().lower()
            if choice in ['yes', 'y']:
                break
            elif choice in ['no', 'n']:
        ...
        viable_size = target_size

    image_height = viable_size
    image_width = viable_size
    ...
    with torch.no_grad():
        for m in modules:
            # Start the background timer for the current module
            timer = ProgressTimer(f"Exporting {m} at {image_width}x{image_height}")
            timer.start()
            ...
            try:
                ...
                # Ensure the timer stops even if an error occurs
                timer.stop()
                
    print("\nAll exports complete!")
    
    # Run the ONNX shape summary automatically after exporting
    print_onnx_summaries(str(output_dir))
